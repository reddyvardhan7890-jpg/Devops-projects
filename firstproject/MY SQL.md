chosen the database MySQL. Hence, we are trying to install it up and configure it.
...
````
```
ps -ef | grep mysql ( to check the running the server)
````
...
````
mysql -h <host-address> -u root -p<password>  --- mysql -u root -pExpenseAPP@1
````


After runnign this command i found that i dont have access so with the help AI i resovled it by using the AI help.
https://claude.ai/share/eeb018fc-d961-4229-980d-57739b3d7c59
